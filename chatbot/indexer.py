...
class CodeIndexer:
    ...
    def index_repository(self):
        ...
        excluded_dirs = {'chroma_db', '__pycache__', '.git', 'node_modules', 'venv', 'env', '.venv', 'static/vendor', 'static/images', 'static/fonts'}
    ...
    def update_index(self, file_path: Path):
        chunks = self.extract_code_info(file_path)
        if not chunks:
            return
        
        ids = [f"{chunk['file']}:{chunk['type']}:{chunk['name']}" for chunk in chunks]
