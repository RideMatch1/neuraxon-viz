...
class CodeRetriever:
    def __init__(self, db_path: Path = None):
        ...
        # FAISS database must be in the deployment (committed to git)
        self.db_path = db_path or (Path(__file__).parent.parent / 'faiss_db')
        
        # Don't try to create directory in read-only filesystem (Vercel)
        # Only create if it doesn't exist and we're not in serverless
        is_serverless = os.environ.get('VERCEL') or os.environ.get('AWS_LAMBDA_FUNCTION_NAME') or os.environ.get('LAMBDA_TASK_ROOT')
        
        if not self.db_path.exists() and not is_serverless:
            # Local development: create directory
            try:
                self.db_path.mkdir(exist_ok=True)
            except (OSError, PermissionError):
                # If we can't create, assume read-only filesystem
                pass
        ...
        # Load FAISS index (read-only, should work in serverless)
        self.index = faiss.read_index(str(self.index_file))
        
        # Load metadata (read-only, should work in serverless)
